...
@Component({
  selector: 'app-unit-add-pop-up',
  templateUrl: './unit-add-pop-up.component.html',
  styleUrls: ['./unit-add-pop-up.component.css']
})
export class UnitAddPopUpComponent implements OnInit {
  name:string
  surname:string
  nameList:string[]=[]
  ...
  add(name){
    if(this.nameInputCheckforRegistration(name)==true){
      this.inputJustifierService.InputCheck("addUnit",null,"add",this.unitObjfortheRegistration).subscribe(data=>{
        alert(data)
      })
